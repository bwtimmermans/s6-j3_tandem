# source("/home/ben/research/NOC/projects/s6-j3_tandem/analysis/coloc_plot_scatter_NDBC.R")
   set.seed(23458892)
   require(lmodel2)
   require(viridis)
...
# Find distance from buoy. Loop over data sets.
      ...
      for (S_idx in 1:3) {
         ...
         list_breaks_master[[S_idx]] <- list_nc1_breaks
      }
#-------------------------------------------------------------------------------------------------#
# Find the "mean" time stamp for each track segment (J3,S6).
      list_breaks <- list_breaks_master
