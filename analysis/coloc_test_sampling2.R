...
#-------------------------------------------------------------------------------------------------#
         ...
         array_list_XXing <- array(list(),dim=c(13,3,length(b_idx_list)))
...
# Find distance from buoy for all points.
               list_B_mat_list_1Hz_dist <- mat_list_1Hz_dist[[m_idx,S_idx]] <- apply(X=cbind(list_buoy_data1[[S_idx]][[2]],list_buoy_data1[[S_idx]][[3]]-360),MAR=1,FUN=func_buoy_dist,B_idx=buoy_idx)
...
# Find the longest track to test against, and label this "1".
            ...
            vec_XXing_temp[ tr_A_idx[tr_A_long_idx] ] <- paste("A",tr_lab_idx,sep="")
            list_tr_test[[tr_lab_idx]] <- vec_tr_test
# Loop over all A tracks that are not the longest (for perform comparison).
...
# It's not inconceivable that the first index does not match any points in the longest segment.
# Perhaps we just keep looping until we get a match.
               Lvec_match <- rep(TRUE,5)
               #for ( tr_p in 1:length(vec_tr_test) ) {
               for ( tr_p in 1:5 ) {
                  print(paste("tr_p:",tr_p))
                  vec_tr_test_p <- cbind(
                                      mat_list_buoy_data1[[m_idx,S_idx]][[3]][mat_list_Lvec_buoy_samp[[m_idx,S_idx]]][vec_tr_test[tr_p]],
                                      mat_list_buoy_data1[[m_idx,S_idx]][[2]][mat_list_Lvec_buoy_samp[[m_idx,S_idx]]][vec_tr_test[tr_p]] )
# For all tracks excluding the longest, cycle through 1 Hz points (index yy) until a match is found.
                  #tr_p_dist <- 1; yy <- 1; while( tr_p_dist > 0.05 ) { tr_p_dist <- sum( abs( mat_tr_lonlat[yy,] - vec_tr_test_p ) ); print(tr_p_dist); yy <- yy + 1; if (yy > 30) { tr_p_dist <- 0; Lvec_match[tr_p] <- FALSE } }
                  tr_p_dist <- 1; yy <- 1; while( tr_p_dist > 0.05 ) { tr_p_dist <- sum( abs( mat_tr_lonlat[yy,] - vec_tr_test_p ) ); yy <- yy + 1; if (yy > dim(mat_tr_lonlat)[1]) { tr_p_dist <- 0; Lvec_match[tr_p] <- FALSE } }
               }
               if ( ! any(Lvec_match) ) {
                  vec_XXing_temp[tr_idx] <- paste(tr_ch,tr_lab_idx+1,sep="")
                  list_tr_test[[tr_lab_idx]] <- mat_list_breaks_master[[m_idx,S_idx]][ tr_idx ]
               } else {
